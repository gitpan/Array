...
sub create_files
{
    ...
    $timestamp = time();  @vocab_array = ();

    #  Set the file names of the internal files
    #  that will be used by the perl module.
    $VOCAB_FILE    = $dest . ".vocab."    . $timestamp;
    $SNT_FILE      = $dest . ".snt."      . $timestamp;
    $SNTNGRAM_FILE = $dest . ".sntngram." . $timestamp;
  
    #  Set default ngram file name if it is not specified.
    if($NGRAM_FILE eq "") { $NGRAM_FILE    = $dest . ".ngram."     . $timestamp; }
...
=head1 Array::Suffix Requirements
   ...
   #  get the ngrams; the output will be stored in
   #  'my_file.txt.ngram.<timestamp>'
   $sarray->get_ngrams();

=head1 Array::Suffix Functions
...
14. set_ngram_file(FILE)
    
    Prints the ngrams to FILE. If this is not set
    the default is: <CORPUS FILE>.ngram.<timestamp>

    NOTE: if an array of corpus files is defined,
    the default is the first corpus file.
